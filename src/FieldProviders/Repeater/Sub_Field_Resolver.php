...
final class Sub_Field_Resolver {
	 ...
	 * @var string
	 */
	const CTRL_SUB_FIELD = 'lpdfe_repeater_sub_field';
	 ...
	 *
	 * Strategy:
	 *  1. If current global $post is our virtual post carrying pre-loaded lpdfe_row_data,
	...
	public static function resolve( string $sub_field_key ) {
		...
		if ( isset( $post->lpdfe_provider_key ) && 'repeater' === $post->lpdfe_provider_key && isset( $post->lpdfe_row_data ) && is_array( $post->lpdfe_row_data ) ) {
			...
			if ( isset( $post->lpdfe_row_data[ $sub_field_name ] ) ) {
				return $post->lpdfe_row_data[ $sub_field_name ];
			}

			if ( isset( $post->lpdfe_row_data[ $sub_field_key ] ) ) {
				return $post->lpdfe_row_data[ $sub_field_key ];
			}
		}
